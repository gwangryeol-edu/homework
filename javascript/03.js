...
// 화살표 함수 정의 코드
const sumPositiveNumbersArrow = (arr) => {
  let positive = 0;
...
function classifyStudentsByGrade(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].score >= 90) {
      Aarr.push(arr[i].name);
    } else if (arr[i].score >= 80) {
      Barr.push(arr[i].name);
    } else if (arr[i].score >= 70) {
      Carr.push(arr[i].name);
    } else {
      Darr.push(arr[i].name);
    }
  }
  return {
    A: Aarr,
...
function countElements(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let el = arr[i]; // 현재 요소
    if (count[el]) {
      // count 객체 안에 el이라는 "키"가 존재하는지 확인하는 조건문
      // 존재한다면(중복이라면): value깂 1추가
      count[el]++;
      // 존재하지 않다면(중복이 아니라면): 배열에 key값을 push하고 value값은 1로 초기화
    } else {
      count[el] = 1;
    }
  }
