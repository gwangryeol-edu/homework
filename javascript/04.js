...
/* 로직 코드 작성 */
let highPriorityTodos = originTodos.filter((pri) => pri.priority === 0);
// 출력 코드
...
/* 로직 코드 작성 */
let incompleteTodos = originTodos.filter((pri) => pri.completed === false);
...
let todosWithPriorityLabels = originTodos.map((todo) => {
  // 우선순위에 맞는 라벨 찾기
  const label = priorityLabels[todo.priority];
...
/* 로직 코드 작성 */
let todoById3 = originTodos.find((todo) => {
  if (todo.id === 3) {
    return todo.text;
  } else {
    false;
...
let todoById10 = originTodos.find((todo) => {
  if (todo.id === 10) {
    return todo.text;
  } else {
    false;
...
/* 로직 코드 작성 */
let allTodosCompleted = originTodos.every((todo) => {
...
let hasUrgentTodos = originTodos.some((todo) => {
...
let completedCount = originTodos.filter(
...
// slice() -> 월본 배열을 복사(원본 유지)
let sortedTodos = originTodos.slice().sort((a, b) => {
